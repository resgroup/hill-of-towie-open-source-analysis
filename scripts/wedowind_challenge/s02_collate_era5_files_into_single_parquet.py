...
"""Convert ERA5 downloaded data to parquet data.
...
`uv run scripts/wedowind_challenge/s02_collate_era5_files_into_single_parquet.py`
"""
...
if __name__ == "__main__":
    ...
    for file_path in chosen_dir.glob("*.grib"):
        ...
        for (lat, lon), sub_df in df.groupby(["latitude", "longitude"]):
            ...
            _df = pd.DataFrame(
                {
                    "ERA5_wind_speed_at_100m_mps": ws,
                    "ERA5_wind_direction_at_100m_deg": wd,
                    "ERA5_temperature_at_10m_degC": _convert_temperature_to_celsius(sub_df["t2m"].to_numpy()),
                },
                index=sub_df.index.get_level_values("time")  # type: ignore[attr-defined]
