...
def _augment_with_era5_data(df: pl.LazyFrame, era5_df: pl.LazyFrame) -> pl.LazyFrame:
    non_timestamp_cols = [c for c in era5_df.collect_schema().names() if c != "TimeStamp_StartFormat"]
    return df.join(era5_df, on=["TimeStamp_StartFormat"], how="left").with_columns(
        pl.col(c).fill_null(strategy="forward") for c in non_timestamp_cols
    )
...
if __name__ == "__main__":
    ...
    full_dataset = (
        ...
        .with_columns(
            target=pl.when(pl.col("is_valid"))
            .then(pl.col(f"wtc_ActPower_mean;{target_turbine}").clip(lower_bound=0))
            .otherwise(None),
        )
    )
