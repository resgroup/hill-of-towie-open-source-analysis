...
from hot_open.settings import get_wind_up_output_dir, get_out_dir
from hot_open.unpack import unpack_local_scada_data_v2, unpack_local_meta_data
...
def _extract_hot_wake_steers_from_table(wakesteer_table: pd.DataFrame) -> list[HOTWakeSteer]:
    ...
    for row in wakesteer_table.itertuples():
        if abs(row.YawOffset_deg) > 0 and not found_a_steer:
            ...
            upwind_wtg = "HOT_" + row.TurbineName
            downwind_wtg = "HOT_" + row.DependentTurbineNames
    ...
    for wake_steer in wake_steers:
        wake_steer_df = wakesteer_table[
            (wakesteer_table["TurbineName"] == wake_steer.upwind_wtg.replace("HOT_", ""))
            & (wakesteer_table["DependentTurbineNames"] == wake_steer.downwind_wtg.replace("HOT_", ""))
        ]
