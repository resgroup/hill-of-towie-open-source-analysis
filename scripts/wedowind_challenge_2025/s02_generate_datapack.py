...
`uv run scripts/wedowind_challenge/s02_generate_datapack.py`
"""
...
from hot_open.settings import get_data_dir, get_wind_up_output_dir
...
def _augment_with_era5_data(df: pl.LazyFrame, era5_df: pl.LazyFrame) -> pl.LazyFrame:
    era5_cols = era5_df.collect_schema().names()
    ts_col = "TimeStamp_StartFormat"
    renaming_ts_col = {era5_cols[0]: ts_col}
    renaming_non_ts_cols = {c: f"ERA5_{c}" for c in era5_cols[1:]}
    renaming_map = renaming_ts_col | renaming_non_ts_cols
    prepped_era5_df = era5_df.rename(renaming_map).with_columns(pl.col(ts_col).dt.cast_time_unit("us"))
    return df.join(prepped_era5_df, on=[ts_col], how="left").with_columns(
        pl.col(c).fill_null(strategy="forward") for c in renaming_non_ts_cols.values()
    )
...
if __name__ == "__main__":
    ...
    output_dir = get_wind_up_output_dir() / "wedowind_competition_input_data"
    ...
    # fetching and processing ERA5 data generated by `s01_download_reference_data.py`
    era5_df = pl.scan_parquet(output_dir / "ERA5_57.50_-3.25.parquet")
